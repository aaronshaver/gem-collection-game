...
struct GameNavigationBar: View {
    ...
    var body: some View {
        HStack(spacing: 4) {
            tab("Collection", symbol: collectionSelected ? "trophy.fill" : "trophy",
    ...
    private func tab(_ title: String, symbol: String, selected: Bool = false,
                     unread: Bool = false, action: @escaping () -> Void) -> some View {
        Button(action: action) {
            Image(systemName: symbol)
                ...
                .background(selected ? Color.mint.opacity(0.13) : .clear,
